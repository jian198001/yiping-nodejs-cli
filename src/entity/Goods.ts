...
export class Goods extends BaseModel {
  ...
  @ApiProperty({
    description: "保证金金额(元),大于100,标识符名称来自淘宝开放平台",
  })
  ...
  @ApiProperty({
    description:
      "重复上拍次数,默认为0，0表示不自动重新上架,标识符名称来自淘宝开放平台",
  })
  public repeatTimes: number;
  @Column({
    nullable: true,
    comment: "",
    name: "delivery_template_locale_valuation",
  })
  ...
  @Column({
    nullable: true,
    comment: "",
    name: "delivery_template_locale_name",
  })
  ...
  @ApiProperty({
    description: "限购数,0表示不限购,默认值为0,标识符名称来自有赞",
  })
  ...
  @ApiProperty({
    description:
      "商品新旧程度：5:全新，6：二手，8:闲置,标识符名称来自淘宝开放平台",
  })
  public stuffStatus: string;
  @Column({ nullable: true, comment: "", name: "link_man" })
  @ApiProperty({
    description: "联系人姓名,最多30个字符,标识符名称来自淘宝开放平台",
  })
  public linkMan: string;
  @Column({ nullable: true, comment: "", name: "unit" })
  @ApiProperty({
    description:
      "计量单位,默认值:item件,可选值:质量mass(kg),体积volume(毫升ml),时间time(分minute),距离length(公里km),面积area(平方米m2)",
  })
  ...
  @ApiProperty({
    description:
      "商品上传后的状态。可选值:onsale(出售中),instock(仓库中);默认值:instock",
  })
  ...
  @ApiProperty({
    description:
      "宝贝描述。字数要大于5个字符，小于25000个字符，受违禁词控制,标识符名称来自淘宝开放平台",
  })
  public desc: string;
}
