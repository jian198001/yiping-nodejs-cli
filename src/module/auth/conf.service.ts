...
export class ConfService extends BaseService {
  ...
  // 查询的数据库表名称
  private static TABLE_NAME = 'conf';
  // 查询的数据库表名称及别名
  private fromSql = ` FROM ${ConfService?.TABLE_NAME} t `;
  // 查询的字段名称及头部的SELECT语句
  ...
  private repository: Repository<Conf> = null;
  /**
   * 分页查询配置
   * @param query - 查询条件字符串
  ...
  public async getById(id = ""): Promise<any> {
    ...
    const key = ConfService.TABLE_NAME + `:${id}`;
...
  }
  ...
  public async update(obj: Conf): Promise<Conf> {
    ...
    // 字段非重复性验证
    const uniqueText = await super.unique?.(ConfService?.TABLE_NAME, [], obj?.id); // 新增或修改数据时，判断某字段值在数据库中是否已重复
