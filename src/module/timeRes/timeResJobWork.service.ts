...
export class TimeResJobWorkService extends BaseService {
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = TimeResJobWorkService?.TABLE_NAME + `:arr`;  
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, TimeResJobWorkService?.TABLE_NAME);          
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    this?.redisService?.del?.(TimeResJobWorkService?.TABLE_NAME + `:arr`);        
  }
