...
export class TabbarService extends BaseService {
  ...
  public async page(
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, TabbarService?.TABLE_NAME);       
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    // 删除redis缓存
    this?.redisService?.del?.(TabbarService?.TABLE_NAME + `:arr`);                
  }
  ...
  public async update(obj: Tabbar): Promise<any> {
    ...
    if (!obj?.id) {
      ...
      if (!obj?.orderNum) {
        await super.sortOrder?.(obj?.id, null, null, TabbarService?.TABLE_NAME); // 新增数据时，设置此条数据的orderNum排序值
      }
       return {} ;
    }

    let old: Tabbar = await this?.repository?.findOneById?.(obj?.id); // 新增或修改数据时，先根据id查询,如此id在数据库中不存在，则是新增，如已存在，则是修改
