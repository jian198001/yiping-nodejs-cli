...
@Provide()
export class FavorService extends BaseService { // 收藏服务
  // 查询的数据库表名称
...
// 执行查询语句并返回page对象结果
  ...
  public async update(obj: Favor): Promise<Favor> {// 上面是验证，下面是数据更新 -- 支持3种情况: 1. 新增数据,主键由前端生成 2. 新增数据，主键由后端生成 3. 修改数据，主键由前端传递
    if (!obj?.id) {
    ...
    if (!obj?.orderNum) {
      await super.sortOrder?.(obj?.id, null, null, FavorService?.TABLE_NAME, ) // 新增数据时，设置此条数据的orderNum排序值
    }
