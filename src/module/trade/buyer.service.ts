...
@Provide()
export class BuyerService extends BaseService { // 买家服务
...
  }
  ...
  public async update(obj: Buyer): Promise<Buyer> {
    // 一个表进行操作 typeORM

    let log = '';

   // 字段非重复性验证
   const uniqueText = await super.unique?.(BuyerService?.TABLE_NAME, [], obj?.id); // 新增或修改数据时，判断某字段值在数据库中是否已重复
...
// 上面是验证，下面是数据更新 -- 支持3种情况: 1. 新增数据,主键由前端生成 2. 新增数据，主键由后端生成 3. 修改数据，主键由前端传递
  ...
  public async loginGoogle(
    ...
    if (!result) {
      ...
      userOpenId.openId = userInfo?.googleId;
