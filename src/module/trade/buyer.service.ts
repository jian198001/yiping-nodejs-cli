...
export class BuyerService extends BaseService {
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = BuyerService?.TABLE_NAME + `:arr`;  
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, BuyerService?.TABLE_NAME);                
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    this?.redisService?.del?.(BuyerService?.TABLE_NAME + `:arr`);      
  }
  ...
  public async update(obj: Buyer): Promise<any> {
    ...
    // 删除redis缓存
    const key = BuyerService?.TABLE_NAME + `:${obj?.id}`;
    await this?.redisService?.del?.(key);   
