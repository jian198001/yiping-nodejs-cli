...
export class CashCardService extends BaseService {
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = CashCardService?.TABLE_NAME + `:arr`;  
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, CashCardService?.TABLE_NAME);                     
  ...
  public async getById(id = ""): Promise<any> {
    ...
    const key = CashCardService.TABLE_NAME + `:${id}`;
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    // 删除redis缓存
    this?.redisService?.del?.(CashCardService?.TABLE_NAME + `:arr`);     
  }
   ...
   * @returns 更新后的现金卡对象
   */
  public async update(obj: CashCard): Promise<any> {
    // 一个表进行操作 typeORM

    let log = "";
    // 删除redis缓存

    const key = CashCardService?.TABLE_NAME + `:${obj?.id}`;
