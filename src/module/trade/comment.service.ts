...
export class CommentService extends BaseService {
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = CommentService?.TABLE_NAME + `:arr`;  
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, CommentService?.TABLE_NAME);                      
  ...
  public async getById(id = ""): Promise<any> {
    ...
    const key = CommentService.TABLE_NAME + `:${id}`;
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    // 删除redis缓存
    this?.redisService?.del?.(CommentService?.TABLE_NAME + `:arr`);     
  }
   ...
   */
  public async update(obj: Comment): Promise<any> {
    // 一个表进行操作 typeORM

    let log = ""; 

    // 删除redis缓存
    const key = CommentService?.TABLE_NAME + `:${obj?.id}`;
    await this?.redisService?.del?.(key);   
