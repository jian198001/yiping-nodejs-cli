...
export class DeliveryCompanyService extends BaseService {
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = DeliveryCompanyService?.TABLE_NAME + `:arr`;  
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, DeliveryCompanyService?.TABLE_NAME);                    
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    this?.redisService?.del?.(DeliveryCompanyService?.TABLE_NAME + `:arr`);      
  }
