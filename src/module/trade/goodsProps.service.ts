...
export class GoodsPropsService extends BaseService {
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = GoodsPropsService?.TABLE_NAME + `:arr`;
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, GoodsPropsService?.TABLE_NAME);                          
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    this?.redisService?.del?.(GoodsPropsService?.TABLE_NAME + `:arr`); 
  }
   ...
   * @returns 更新后的商品属性对象
   */
  public async update(obj: GoodsProps): Promise<any> {
    // 一个表进行操作 typeORM

    let log = "";
    // 删除redis缓存
    const key = GoodsPropsService?.TABLE_NAME + `:${obj?.id}`;
    await this?.redisService?.del?.(key); 
