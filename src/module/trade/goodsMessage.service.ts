...
export class GoodsMessageService extends BaseService {
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = GoodsMessageService?.TABLE_NAME + `:arr`;
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, GoodsMessageService?.TABLE_NAME);                          
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    this?.redisService?.del?.(GoodsMessageService?.TABLE_NAME + `:arr`);   
  }
   ...
   * @returns 更新后的商品消息对象
   */
  public async update(obj: GoodsMessage): Promise<any> {
    // 一个表进行操作 typeORM
