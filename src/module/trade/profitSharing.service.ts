...
export class ProfitSharingService extends BaseService {
  ...
  // 查询的数据库表名称
  private static TABLE_NAME = "profit_sharing";

  // 查询的数据库表名称及别名
  private fromSql = ` FROM ${ProfitSharingService?.TABLE_NAME} t `;
  // 查询的字段名称及头部的SELECT语句
  private selectSql = ` ${BaseService.selSql}  
  
     `;

  @InjectEntityModel(ProfitSharing)
  private repository: Repository<ProfitSharing> = null;
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = ProfitSharingService?.TABLE_NAME + `:arr`;
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, ProfitSharingService?.TABLE_NAME);     
  ...
  public async getById(id = ""): Promise<any> {
    ...
    const key = ProfitSharingService?.TABLE_NAME + `:${id}`;
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    this?.redisService?.del?.(ProfitSharingService?.TABLE_NAME + `:arr`);  
  }
   ...
   */
  public async update(obj: ProfitSharing): Promise<any> {
    // 一个表进行操作 typeORM

    let log = "";
    // 删除redis缓存

    const key = ProfitSharingService?.TABLE_NAME + `:${obj?.id}`;
    ...
    const uniqueText = await super.unique?.(
      ProfitSharingService?.TABLE_NAME,
      [],
      obj?.id
    );
    ...
    let old: ProfitSharing = await this?.repository?.findOneById?.(obj?.id); // 新增或修改数据时，先根据id查询,如此id在数据库中不存在，则是新增，如已存在，则是修改
