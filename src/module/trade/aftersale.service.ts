...
export class AftersaleService extends BaseService {
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = AftersaleService?.TABLE_NAME + `:arr`;  
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, AftersaleService?.TABLE_NAME);              
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    // 删除redis缓存
    this?.redisService?.del?.(AftersaleService?.TABLE_NAME + `:arr`);      
  }
  /**
   * 更新售后服务记录
   * @param obj - 售后服务记录对象
