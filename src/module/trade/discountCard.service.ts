...
export class DiscountCardService extends BaseService {
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = DiscountCardService?.TABLE_NAME + `:arr`;   
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, DiscountCardService?.TABLE_NAME);                  
  ...
  public async getById(id = ""): Promise<any> {
    ...
    const key = DiscountCardService.TABLE_NAME + `:${id}`;
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    this?.redisService?.del?.(DiscountCardService?.TABLE_NAME + `:arr`);   
  }
   ...
   * @returns 更新后的优惠卡对象
   */
  public async update(obj: DiscountCard): Promise<any> {
    // 一个表进行操作 typeORM

    let log = "";
    // 删除redis缓存

    const key = DiscountCardService?.TABLE_NAME + `:${obj?.id}`;
