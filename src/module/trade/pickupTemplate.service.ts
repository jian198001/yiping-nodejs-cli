...
export class PickupTemplateService extends BaseService {
  ...
  public async page(
    ...
    if (page?.pageSize < 1) {
      ...
      const key = PickupTemplateService?.TABLE_NAME + `:arr`;
    ...
    // 将查询结果中的数据列表存入redis
    this?.setArrToRedis?.(data?.list, PickupTemplateService?.TABLE_NAME);    
  ...
  public async del(ids: string[]): Promise<void> {
    ...
    // 删除redis缓存
    this?.redisService?.del?.(PickupTemplateService?.TABLE_NAME + `:arr`); 
  }
   ...
   */
  public async update(obj: PickupTemplate): Promise<any> {
    // 一个表进行操作 typeORM
