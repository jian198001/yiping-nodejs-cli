 ...
 * 买家前端页面通行证控制器
 */
@Controller("/buyer/uni/frontPage/timeRes/passport")
export class BuyerUniFrontPagePassportPassportController {
  ...
  public async buyerLogin(
    ...
    // 调用买家服务的买家登录方法
    data = await this?.buyerService?.buyerLogin(buyer, shopId);
  ...
  public async sellerLogin(
    ...
    // 调用买家服务的卖家登录方法
    data = await this?.buyerService?.sellerLogin(buyer, shopId);
  ...
  public async loginGoogle(@Body() body: GoogleCredentials): Promise<any> {
    // 记录日志
    this?.logger?.info?.("登录controller");
